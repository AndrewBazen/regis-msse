...
After two assignments on Replit, I wanted to compare a different style of "vibe coding". Also this was necessary since I was completely out of agent credits on replit:

- **Local-first, available credits** Replit's Agent credits ran out at the end of
  Assignment 2 after repeated revisions. Claude Code runs against my own files on disk, so I had to use claude code since I have credits already on it.
- **It owns the whole project, not just a chat box.** The agent read my existing repo,
  ...
  rendered — all without me leaving the terminal.  Personally, I really enjoy being able to see the code.
- **The deliverable is plain static files.** This game has no real backend (the "vulnerable
  ...
  right fit. It opens `index.html` straight in the browser.

The trade-off: Replit gives you a live public URL out of the box, while my Claude Code build
runs locally. However, it would be easy to host this as a site.
...
"Forbidden", and the whole game is about reaching things that should have returned 403 but
don't.
...
- **The Inspector** — the live request the browser is about to send: the URL, HTTP method, cookies, decoded JWT, and request body where every field is editable.
...
The loop itself is deliberately simple: **change the appropriate item in the Inspector, press Send
...
7. **JWT `alg:none`.** The API accepts unsigned tokens. Rewrite the payload to `role: admin`,
   set the header `alg` to `none`, and delete the signature to forge an admin token. There is 
   a JWT primer before this level, see below for screenshot.
...
*authentication*, which is **who** you are, which is what I used for Assignment 1.) Broken Access Control is
...
remains at the top of the 2025 list.
...
the next handler. The defensive principle is to **refuse every request by default**
unless an explicit, server-side rule allows it, and **never trust the client** for anything
that decides authorization.

### Recent incidents

- **Optus (2022).** An **unauthenticated, internet-exposed API
...
**Problem 1: Wrong answers risked being unhelpful "403"s.**
...
*Fix:* I had the agent rewrite each level to branch on the *kind* of wrong answer and return a
specific diagnostic.  Every wrong attempt now names what's still missing.

**Problem 2: How to show the player what to adjust**
Showing the URL, method, cookies, JWT, and body on every level made it
unclear where and how the player should act.

*Fix:* I made the agent make the Inspector **level-aware**: each level declares which pane is relevant, and the
engine expands exactly that pane by default while leaving the others collapsed but available.

**Problem 3: JWTs are not understood by everyone playing the game**
Level 7 asks the player to hand-edit a token's header and payload, which assumes they know what
those are.

*Fix:* I had the agent add a **30-second JWT primer** that fires automatically just before Level 7,
explaining the header/payload/signature structure.
...
*Fix:* I directed the agent to create a **per-level pre-brief** where every level now
opens with a modal that teaches the vulnerability concept before the player touches anything —
and add a one-time **"How to play" tutorial** that explains the three-panel layout and the
